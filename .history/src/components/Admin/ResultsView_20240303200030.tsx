...
const ResultsPage: React.FC<Props> = ({navigation}) => {
  ...
  const fetchVoteResults = async () => {
    try {
      ...
      // yesterday's votes
      const yesterdayVotesQuery = query(
        votesCollection,
        where('voteDate', '==', yesterdayString),
      );
      ...
      const sortedYesterdayOptions = Object.keys(yesterdayOptions).sort(
        (a, b) => yesterdayOptions[b] - yesterdayOptions[a],
      );
      await fetchFoodOptions(
        sortedYesterdayOptions.slice(0, 2),
        setTopOptionsYesterday,
      );

      //  today's votes
      const todayVotesQuery = query(
        votesCollection,
        where('voteDate', '==', today),
      );
      ...
      const sortedTodayOptions = Object.keys(todayOptions).sort(
        (a, b) => todayOptions[b] - todayOptions[a],
      );
      await fetchFoodOptions(
        sortedTodayOptions.slice(0, 2),
        setTopOptionsToday,
      );
  ...
  const fetchFoodOptions = async (
    options: string[],
    setOptions: React.Dispatch<React.SetStateAction<FoodOption[]>>,
  ) => {
    ...
    try {
      ...
      for (const option of options) {
        const response = await fetch(
          `https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKey}&number=1&addRecipeInformation=true&query=${option}&sort=popularity`,
        );
  ...
  const renderVoteResults = () => {
    if (loading) {
      ...
      return (
        <View>
            <Text style={styles.subTitle}>Top options voted yesterday:</Text>
          ...
          <Text style={styles.subTitle}> Top options voted today:</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
            <Text style={styles.title}>Students' Voting Results</Text>
            <Button onPress={() => FIREBASE_AUTH.signOut()} title="Log ot" />
      </View>
      <View>{renderVoteResults()}</View>
    </View>
  );
};
