...
const ViewInstructions: React.FC<ViewInstructionsProp> = ({ route }) => {
      ...
      const fetchFoodDetails = async () => {
        try {
          ...
          const response = await fetch(
            `https://api.spoonacular.com/recipes/recipes?apiKey=${apiKey}&number=1&addRecipeInformation=true&query=${foodOptionId}&sort=popularity`
          );
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.detail}>{foodDetails.summary}</Text>
