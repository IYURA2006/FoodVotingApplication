...
const ProfilePage = ({navigation}: RouterProps) => {
  ...
  const [isDateOfBirthEditMode, setIsDateOfBirthEditMode] = useState(false);
      ...
      try {
        ...
        if (user) {
          ...
          if (userDocSnap.exists()) {
            ...
            setDateOfBirth(new Date(userDocSnap.data().dateOfBirth));
          } else {
            ...
            <>
              ...
              <View style={styles.editableItemContainer}>
                ...
                {!isSurnameEditMode && (
                  <Button
                    onPress={() => {
                      setEditableItem('surname');
                      setIsSurnameEditMode(true);
                    }}
                    title="Edit"
                  />
                )}
  ...
  {editableItem === 'dateOfBirth' ? (
    <TouchableOpacity onPress={() => setIsDateOfBirthEditMode(true)} style={styles.editableItem}>
      <Text>{dateOfBirth.toLocaleDateString()}</Text>
    </TouchableOpacity>
  ) : (
    <Text style={styles.text}>{dateOfBirth.toLocaleDateString()}</Text>
  )}
  {!isDateOfBirthEditMode && <Button onPress={() => { setEditableItem('dateOfBirth'); setIsDateOfBirthEditMode(true); }} title="Edit" />}
  {isDateOfBirthEditMode && (
    <>
      <DatePicker
        date={dateOfBirth}
        ...
        mode="date"
      />
      <Button onPress={() => { setEditableItem(null); setIsDateOfBirthEditMode(false); }} title="Cancel" />
      <Button onPress={() => { setEditableItem(null); setIsDateOfBirthEditMode(false); handleUpdateProfile(); }} title="Submit" />
    </>
  )}
</View>
              ...
              <View style={styles.editableItemContainer}>
                ...
                {!isCurrentYearEditMode && (
                  <Button
                    onPress={() => {
                      setEditableItem('currentYear');
                      setIsCurrentYearEditMode(true);
                    }}
                    title="Edit"
                  />
                )}
                {isCurrentYearEditMode && (
                  <>
                    <Button
                      onPress={() => {
                        setEditableItem(null);
                        setIsCurrentYearEditMode(false);
                      }}
                      title="Cancel"
                    />
                    <Button
