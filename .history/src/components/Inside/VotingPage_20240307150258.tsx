...
import { getDocs, collection, addDoc, getDoc, doc, serverTimestamp, where, query } from 'firebase/firestore';
...
const VotingPage: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchFoodOptionsSpoonacular = async (query: string) => {
    try {
      const apiKey = '344d206da412479ab23f0d3f2572976';
  ...
  useEffect(() => {
    const fetchDailyOptions = async () => {
      try {
        ...
        const dailyOptionsQuery = query(collection(FIRESTORE_DB, 'dailyOptions'), where('date', '==', today));
        const dailyOptionsSnapshot = await getDocs(dailyOptionsQuery);

        if (dailyOptionsSnapshot.empty) {
          // If no daily options available for today, generate new ones
          const newDailyOptions = await generateDailyOptions(today);
          setFoodOptions(newDailyOptions);
        } else {
          // If daily options exist, fetch and set them
          const dailyOptionsData = dailyOptionsSnapshot.docs[0].data();
          setFoodOptions(dailyOptionsData.options);
        }
      } catch (error) {
        console.error('Error fetching daily options:', error);
      }
    };
  ...
  const generateDailyOptions = async (date: string) => {
    try {
      const foodOptionsSnapshot = await getDocs(collection(FIRESTORE_DB, 'foodOptions'));
      ...
      const selectedFoodOptions: FoodOption[] = [];
      const selectedFoodOptionIds = new Set<string>();

      while (selectedFoodOptions.length < 4) {
        const shuffledFoodOptions = [...foodOptionsData].sort(() => Math.random() - 0.5);
        const selectedFoodOption = shuffledFoodOptions.find(
          (option) => !selectedFoodOptionIds.has(option.id)
        );

        if (selectedFoodOption) {
          selectedFoodOptions.push(selectedFoodOption);
          selectedFoodOptionIds.add(selectedFoodOption.id);
        }
      }

      // Add daily options to Firestore
      const dailyOptionsRef = collection(FIRESTORE_DB, 'dailyOptions');
      await addDoc(dailyOptionsRef, { date, options: selectedFoodOptions });

      return selectedFoodOptions;
    } catch (error) {
      console.error('Error generating daily options:', error);
      return [];
    }
  };
