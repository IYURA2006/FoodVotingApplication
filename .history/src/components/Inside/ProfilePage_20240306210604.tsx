...
const ProfilePage = ({ navigation }: RouterProps) => {
  ...
  // Options for houses and current year
  const houseOptions = ['C1', 'C2', 'C3', "Cotton", "Preshute", "Ivy", "Turner", "Dancy"];
  const currentYearOptions = ['Shell', 'Remove', 'Hundred', 'L6', 'U6'];
  ...
  const handlePasswordChange = async () => {
    try {
      ...
      Alert.alert('Error', 'Failed to update password. Please enter the correct password.');
    }
  };
  ...
  const renderOptionsModal = () => {
    const options = editableItem === 'currentYear' ? currentYearOptions : houseOptions;
